...
const findItem = <T extends { id: String }>(items: any[], id: string): T => {
...
const ProjectGrid = (props: PropsType) => {
  ...
  useEffect(() => {
    if (query.get('id')) {
      ...
      let pd = findItem<ProjectObject>(items, itemId);
  ...
  useEffect(() => {
    ...
    if (type !== '' && type !== 'all') {
      filtered = items.filter((item: ProjectObject) => item.type === type && item.state === 'release');
    } else {
      filtered = items;
    }
    if (top > 0) {
